...
PRO IDLVIEWER_EVENT, ev
  ...
  IF tagName EQ 'WIDGET_KILL_REQUEST' THEN BEGIN
    ...
    ;程序关闭提示
    ret = DIALOG_MESSAGE('确定退出吗？', title = '天绘卫星影像土地覆盖信息提取系统', /question)
  ...
  CASE uname OF
    ...
    'Exit': BEGIN
      ;程序关闭提示
      ret = DIALOG_MESSAGE('确定退出吗？', title = '天绘卫星影像土地覆盖信息提取', /question)
      IF ret EQ 'No' THEN RETURN
      WIDGET_CONTROL, ev.TOP, /destroy
      fids = ENVI_GET_FILE_IDS()
      FOREACH id, fids DO ENVI_FILE_MNG, id = id, /remove
...
PRO IDLVIEWER_USEENVI
  ...
  ;启动界面
  ScreenImage = ImagePath + '\LandCover.jpg'
  data = READ_IMAGE(ScreenImage)
  SplashBase = SHOW_SPLASH_SCREEN(data, /true)
  
  ;搭建界面
  tlb = WIDGET_BASE(title = '天绘卫星影像土地覆盖信息提取',     $
    mbar = mbar,                              $
    uname = 'tlb',                            $
  ...
  ;*************************添加自定义菜单   示例代码*****************************************
  wExample = WIDGET_BUTTON(mbar, /menu, value = '信息提取')
  wSU = WIDGET_BUTTON(wExample, value = '混合像元分解', event_pro = 'Spectral_Unmixing')
  wSU = WIDGET_BUTTON(wExample, value = '参数设置', event_pro = 'Spectral_Unmixing')
  ...
  wAbout = WIDGET_BUTTON(wHelp, value = '关于系统', uname = 'About')
