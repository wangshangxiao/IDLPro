...
PRO XWINDOW_NULL_EVENTS, event
END ; of XWINDOW_NULL_EVENTS event handler ******************************
   ...
   'SELECT_FILE': BEGIN
    ...
    'ACCEPT': BEGIN  ; Gather the form information.
       ...
       formdata = {filename:filename, xsize:xsize, ysize:ysize, $
          color:colortype, order:order, quality:quality}
...
FUNCTION XWINDOW_CONFIGURATION, filetype, config, TITLE=title, $
   XOFFSET=xoffset, YOFFSET=yoffset, Cancel=cancel, Configure=configure, $
   PARENT=parent
...
IF N_ELEMENTS(title) EQ 0 THEN BEGIN
   IF Keyword_Set(configure) THEN BEGIN
      title = 'Configure ' + filetype + ' Output File'
   ENDIF ELSE BEGIN
      title = 'Create ' + filetype + ' Output File'
   ENDELSE
ENDIF
...
filenamebase = Widget_Base(filebox, Row=1)
   ...
   filenameID = Widget_Text(filenamebase, Value=filename, /Editable, $
      Event_Pro='XWINDOW_NULL_EVENTS', SCR_XSIZE=320)
...
xsizeID = FSC_INPUTFIELD(sizebase, Value=config.xsize, Title='XSize: ', $
   /IntegerValue, /Positive)
ysizeID = FSC_INPUTFIELD(sizebase, Value=config.ysize, Title='YSize: ', $
   /IntegerValue, /Positive)
...
colordropID = Widget_Droplist(orderbase, Value=type, $
   Title='File Type: ', EVENT_PRO='XWINDOW_NULL_EVENTS')
orderdropID = Widget_Droplist(orderbase, Value=order, $
   Title='Display Order: ', EVENT_PRO='XWINDOW_NULL_EVENTS')
...
IF filetype EQ 'JPEG' THEN $
   sliderID = Widget_Slider(bigbox, Value=config.quality, Max=100, Min=0, $
      Title='Compression Quality', EVENT_PRO='XWINDOW_NULL_EVENTS', $
...
index = Where(tagnames EQ 'PS', count)
IF count GT 0 THEN Obj_Destroy, info.ps
...
IF info.catchit THEN CATCH, error ELSE error = 0
IF error NE 0 THEN BEGIN
   Catch, /Cancel
   ok = DIALOG_MESSAGE(!Error_State.Msg)
...
CASE whichFile OF
   ...
   'Create PostScript File': BEGIN
      info.ps->GUI, Cancel=cancelled, Group_Leader=event.top
      IF cancelled THEN BEGIN
         WIDGET_CONTROL, event.top, SET_UVALUE=info, /NO_COPY
         RETURN
      ENDIF
      keywords = info.ps->GetKeywords()
      thisDevice = !D.NAME
      TVLCT, r, g, b, /GET
      SET_PLOT, 'PS'
      !P.Background = info.background
      TVLCT, info.r, info.g, info.b, info.bottom
      keywords.color = 1 ; Aways turn color on for decent output.
   ...
   'Create GIF File': BEGIN
      newConfiguration = XWINDOW_CONFIGURATION('GIF', info.gif, $
         Cancel=cancelled, Parent=event.top)
      IF NOT cancelled THEN info.gif = newConfiguration ELSE BEGIN
         WIDGET_CONTROL, event.top, SET_UVALUE=info, /NO_COPY
         RETURN
      ENDELSE
   ...
   'Create TIFF File': BEGIN
      ...
      newConfiguration = XWINDOW_CONFIGURATION('TIFF', info.tiff, $
         Cancel=cancelled, Parent=event.top)
      IF NOT cancelled THEN info.tiff = newConfiguration ELSE BEGIN
         WIDGET_CONTROL, event.top, SET_UVALUE=info, /NO_COPY
         RETURN
      ENDELSE
   ...
   'Create JPEG File': BEGIN
      newConfiguration = XWINDOW_CONFIGURATION('JPEG', info.jpeg, $
         Cancel=cancelled, Parent=event.top)
      IF NOT cancelled THEN info.jpeg = newConfiguration ELSE BEGIN
         WIDGET_CONTROL, event.top, SET_UVALUE=info, /NO_COPY
         RETURN
      ENDELSE
...
PRO XWINDOW, proName, param1, param2, param3, GROUP_LEADER=group, $
   ...
   WXPOS=wxpos, WYPOS=wypos, WBACKGROUND=background, Catch=catchit

   ; Catch errors.

IF N_Elements(catchit) EQ 0 THEN catchit = 1
IF Keyword_Set(catchit) THEN CATCH, thisError ELSE thisError = 0
...
info = { xsize:xsize, $                     ; X size of window.
         ...
         catchit:Keyword_Set(catchit), $    ; A CATCH error handler flag.
...
CD, Current=thisDir
ps =   Obj_New('FSC_PSCONFIG', Filename='xwindow.ps')
pslocal = {PS_FORM_INFO, XSIZE:5.0, XOFF:1.75, YSIZE:5.0, $
       YOFF:3.0, FILENAME:FilePath(Root_Dir=thisDir,'xwindow.ps'), $
       INCHES:1, COLOR:0, BITS_PER_PIXEL:8, $
