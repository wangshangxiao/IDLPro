...
IF(remainSecond LT 1) THEN remainSecond=1
...
IF N_Elements(title) EQ 0 THEN title = "统计进度"
;IF N_Elements(color) EQ 0 THEN color = !P.Color
...
self.tlb = Widget_Base(Title=self.title, Column=1,TLB_FRAME_ATTR=1)
self.parent = -1L
ENDIF ELSE BEGIN
IF self.cancelButton THEN modal = 0 ELSE modal = 1
