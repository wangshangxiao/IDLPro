;
; 蒙继华
; 用来作分县的参数统计,实时统计,统计不同等级所占的百分比
; 2006.08.29
; 是在原来过程统计和全球统计模块的基础上进行改进而成的
; 连控件的名称也还大多是_PRO,没有改成_RT
;-----------------------------------------------------------------
...
FUNCTION STAT_RT_COUNTY	,FILE_IN,YEAR,MONTH,DAY	$
	...
	PROGRESSTIMER = OBJ_NEW("SHOWPROGRESS", TLB,/CANCELBUTTON,MESSAGE=temp_str,TITLE='实时监测统计')
	PROGRESSTIMER->START
	;-------------------------------------------------------------------------------
	...
	RESULT= {	$
		...
		ARR_POINTS	:	FLTARR(5)	,$;用来存储每个等级的累加
		ARR_PER		:	FLTARR(5)	,$;用来存储每个等级的累加
		TOTAL_POINTS:	0	$ ;用来存储所有等级的累加
...
;		writeu,lun_3,DATA_LANDUSE
	...
	;-------------------------------------------------------------------------------
	;这两行代码用于进程条的显示
	temp_str='正在入库'+STRTRIM(DATA_TYPE,2)+'数据'
	temp_str=temp_str+string(byte(13))+string(byte(10))
	temp_str=temp_str+'时间:'+STRTRIM(YEAR,2)+'年'+STRTRIM(MONTH,2)+'月'+STRTRIM(DAY,2)+'日'
	PROGRESSTIMER = OBJ_NEW("SHOWPROGRESS", TLB,/CANCELBUTTON,MESSAGE=temp_str,TITLE='实时监测统计')
	PROGRESSTIMER->START
	...
	FOR I=0,NUM_COUNTY-1 DO BEGIN
		...
		RETURN,0
		ENDIF
		PROGRESSTIMER->UPDATE, (FLOAT(I)/NUM_COUNTY * 100.0) ;继续进行
		;(1)耕地
...
FUNCTION CMD_OK_STA_RT_COUNTY, EVENT
    ...
    ENDIF ELSE BEGIN	;批处理
		...
		FOR	I=JULDAY_START,JULDAY_END DO BEGIN
			...
			IF(RESULT EQ 1) THEN BEGIN
				PRINT,STRTRIM(YEAR_1,2)+'年'+STRTRIM(MONTH,2)+'月'+STRTRIM(DAY,2)+'日数据处理成功'
				COUNT_SUCCESS=COUNT_SUCCESS+1
			ENDIF ELSE BEGIN
				PRINT,STRTRIM(YEAR_1,2)+'年'+STRTRIM(MONTH,2)+'月'+STRTRIM(DAY,2)+'日数据处理失败'
			ENDELSE
		ENDFOR
...
pro STAT_RT_COUNTY_event, Event
  ...
  case wTarget of
	...
	Widget_Info(wWidget, FIND_BY_UNAME='CMD_CHANGE_PATH'): begin
    	WIDGET_CONTROL,Event.top,get_UVALUE = pstate
    	widget_control,wWidget,sensitive=0	;杨绍锷添加，20070803
 		FILE_PATH_NEW=DIALOG_PICKFILE( /DIRECTORY,path=zs_out_path,title='请选择目标文件路径', DIALOG_PARENT=Event.id)
 		IF(FILE_PATH_NEW NE '') THEN BEGIN
 	 		WIDGET_CONTROL,(*pstate).TXT_FILE_PATH,Set_value=FILE_PATH_NEW
 		ENDIF
 		widget_control,wWidget,sensitive=1	;杨绍锷添加，20070803
    end
    ...
    Widget_Info(wWidget, FIND_BY_UNAME='CMD_CHOOSE_FILE'): begin
		...
		widget_control,wWidget,sensitive=0	;杨绍锷添加，20070803
	    file=DIALOG_PICKFILE(filter='*.*',path=zs_out_path,title='请选择一个文件', DIALOG_PARENT=Event.id)
...
common current_date, c_year, c_month, c_day
  ...
  STAT_RT_COUNTY = Widget_Base( GROUP_LEADER=BASE_TOP,  $
      ...
      ,SCR_YSIZE=325 ,TITLE='实时监测--统计到县' ,SPACE=3 ,XPAD=3 ,YPAD=3,TLB_FRAME_ATTR =1)
