FUNCTION NQ_classify_GET_IMAGE_INFO,INFILE
...
;       TEXT=DIALOG_MESSAGE(INHEAD+'头文件不存在或文件格式错误,请检查!')
 ...
 ;  读头文件
    ...
    case HeadFileInfo.datatype of
	 ...
	 openr,datalun,INFILE,/GET_LUN
	 dataRESULT = FSTAT(datalun)
	 FREE_LUN,datalun
    if long(dataRESULT.SIZE/d) ne (LONG(HeadFileInfo.samples)*LONG(HeadFileInfo.lines)) then begin
