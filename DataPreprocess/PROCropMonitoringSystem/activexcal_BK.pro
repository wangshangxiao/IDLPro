...
;---
pro ActiveXCal, event;, c_year=c_year,c_month=c_month,c_day=c_day
	CATCH, Error_status
    ...
    wBase = WIDGET_BASE(COLUMN = 1, SCR_XSIZE = 400, $
       TITLE='选择日期', TLB_FRAME_ATTR=1, xoffset=position[0],yoffset=position[1])
    ...
    wYear = WIDGET_LABEL(wSubBase, VALUE = '0000',/DYNAMIC_RESIZE)
;    wSubBase = WIDGET_BASE(wBase, /ROW)
    wVoid = WIDGET_LABEL(wSubBase, value = '-')
    wMonth = WIDGET_LABEL(wSubBase, value = '0',/DYNAMIC_RESIZE)
;    wSubBase = WIDGET_BASE(wBase, /ROW)
    wVoid = WIDGET_LABEL(wSubBase, VALUE = '-')
    wDay = WIDGET_LABEL(wSubBase, VALUE = '0',/DYNAMIC_RESIZE)
