...
;确保输出文件类型的正确性
	...
	progressTimer = Obj_New("ShowProgress", tlb,MESSAGE='数据处理中,请稍候!',TITLE='FPAR计算')
	progressTimer->START
	...
	FPAR=FLOAT(1+inputimage)/FLOAT(1-inputimage)
	...
	Write_tiff,outputfile,FPAR,geotiff=geotiff,/float
...
PRO FPAR_CALCULATION_EVENT,EVENT

	on_error,2

	forward_function FPAR_CAL
...
PRO FPAR_CALCULATION, GROUP_LEADER=wGroup, _EXTRA=_VWBExtra
...
;设置主框架
	TLB_BASE = WIDGET_BASE(GROUP_LEADER=wGroup, UNAME='TLB_BASE' $
      ...
      ,TITLE='FPAR计算' ,SPACE=3 ,XPAD=3 ,YPAD=1  $
      ,COLUMN=1 ,/TLB_KILL_REQUEST_EVENTS,TLB_FRAME_ATTR=1)
		...
		;选择文件组件
			INPUT_BASE = Widget_Base(FILE_BASE,/ROW,UNAME='INPUT_BASE',/align_right)
				INPUT_FIELD = CW_FIELD(INPUT_BASE,UNAME='INPUT_FIELD',TITLE='输入NDVI数据',XSIZE=32 ,YSIZE=1,NOEDIT=1)
			...
			OUTPUT_BASE = Widget_Base(FILE_BASE,/ROW,UNAME='OUTPUT_BASE',/align_right)
				OUTPUT_FIELD = CW_FIELD(OUTPUT_BASE,UNAME='OUTPUT_FIELD',TITLE='输出FPAR文件',XSIZE=32 ,YSIZE=1,NOEDIT=1)
	...
	Xmanager,'FPAR_CALCULATION',TLB_BASE,/NO_BLOCK
