...
PRO XImageBar_Open, event
   ...
   ; Event handler to open and read a data file.
...
thisEvent = Tag_Names(event, /Structure_Name)
   ...
   ; If it is button event, call GetFile.
...
ENDIF
   ...
   ; If it is a GetFile event, read the data file.
...
IF thisEvent EQ 'GETFILE_EVENT' THEN BEGIN
   ...
   *info.image = image
      ...
      ; Display the new data by creating a pseudo event.
