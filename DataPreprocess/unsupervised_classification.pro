...
FUNCTION UNSUPVISED,EVENT
  ...
  Widget_Control,(*PSTATE).CLS_MIN_NUM,get_value=minclsnum;最小分类数
  ...
  Widget_Control,(*PSTATE).CLS_MAX_NUM,get_value=maxclsnum;最大分类数
  ...
  Widget_Control,(*PSTATE).CLS_ITERATION_NUM,get_value=iternum;最大迭代次数
  case 1 of
    iternum le 0 : begin
      iternum=1
  ...
  Widget_Control,(*PSTATE).CLS_CHANGE_THRESHOLD,get_value=threshold;变化阈值范围
...
;------调用ISODATA分类算法--------------------------------------------------------
  ...
  NUM_CLASSES = maxclsnum[0]
  ITERATIONS = iternum[0]
  CHANGE_THRESH = threshold[0]
  MIN_CLASSES = minclsnum[0]


;  print,'CHANGE_THRESH==',CHANGE_THRESH
...
;	dims = [-1, 0, ns-1, 0, nl-1]
	...
	if o_fid eq -1 then begin
		CAUTION = dialog_message('用户中止了操作或分类影像有问题!',title='警告')
		return,0
	endif
...
PRO UNSUPERVISED_CLASSIFICATION_EVENT,EVENT
	...
	case wTarget of
        ...
        ;progressTimer = Obj_New("ShowProgress", tlb,MESSAGE='数据处理中,请稍候!',TITLE='非监督分类') ;新建进度条对象
        ;progressTimer->START
        ;progressTimer->UPDATE, (0.2 * 100.0)  ;更新进度条
    		
    		;STATUS=ISODATA(event)
    		STATUS=UNSUPVISED(event)
...
PRO UNSUPERVISED_CLASSIFICATION,GROUP_LEADER=wGroup, _EXTRA=_VWBExtra
	...
	INPUT_BASE = Widget_Base(FILE_BASE,/ROW,UNAME='INPUT_BASE',SPACE=1,XPAD=0,YPAD=0,/BASE_ALIGN_CENTER)
		...
		INPUT_FIELD = CW_FIELD(INPUT_BASE,UNAME='INPUT_FIELD',XSIZE=30 ,YSIZE=1,TITLE='输入影像',NOEDIT=1,value='D:\IRSA_WU\hunan_sub\imagemaskbycrop\FarmImage.tif')
		...
		pfinfo = {field_id:INPUT_FIELD, filter:'*.*', title:'输入影像文件'}
	...
	OUTPUT_FILE_BASE = Widget_Base(OUTPUT_BASE,/ROW,UNAME='OUTPUT_FILE_BASE',SPACE=1,XPAD=0,YPAD=0,/BASE_ALIGN_CENTER)
		OUTPUT_FIELD = CW_FIELD(OUTPUT_FILE_BASE,UNAME='OUTPUT_FIELD',XSIZE=30 ,YSIZE=1,TITLE='输出影像',NOEDIT=1,value='D:\IRSA_WU\hunan_sub\isodata\isocropimage.tif')
	...
	CLS_NUM_BASE = Widget_Base(CLS_PARA_BASE,UNAME='CLS_NUM_BASE',/ROW,XPAD=5)
		...
		CLS_MIN_NUM = CW_FIELD(CLS_NUM_BASE,UNAME='CLS_MIN_NUM',XSIZE=5,YSIZE=1,TITLE='最小分类数',VALUE=5)
		CLS_MAX_NUM = CW_FIELD(CLS_NUM_BASE,UNAME='CLS_MAX_NUM',XSIZE=5,YSIZE=1,TITLE='最大分类数',VALUE=10)
