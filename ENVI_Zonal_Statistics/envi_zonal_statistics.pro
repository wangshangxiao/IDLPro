...
PRO ENVI_Zonal_Statistics_extensions_init
...
PRO ENVI_Zonal_Statistics_event, ev
...
PRO ENVIZonalStatistics, Raster, shpFile=shpFile, $
  ...
  CASE uniqFlag OF
    ...
    0: BEGIN
      ...
      ;加入COUNTS
      header = [header, 'COUNT']
      h = h + ORDEREDHASH('COUNT', statsCOUNTS)

      ;如果单位为米，则统计面积
      map_info = ENVI_GET_MAP_INFO(fid=fid)
      IF oproj.UNITS EQ 0 THEN BEGIN
        header = [header, 'AREA']
        h = h + ORDEREDHASH('AREA', statsCOUNTS*PRODUCT(map_info.PS))
      ENDIF
